...
description: Lê threads de comentários de uma PR Arco/OlaIsaac, redige rascunhos de réplicas em PT-BR e persiste o resultado como arquivo .md no Obsidian vault em ~/.notes/pr-reviews/ (contexto arco no frontmatter). Não posta nada no GitHub.
user_invocable: true
---
...
> **Eixo achatado:** desde o flattening do vault, reviews e answers vivem todos em `pr-reviews/` na raiz — o repo já está embutido no nome do arquivo. O contexto (`arco`) viaja no frontmatter (campo `context`).

### 9. Linkar review original (opcional)

Procurar no diretório do vault (filtrando pelo repo no nome do arquivo):

```bash
ls "$NOTES_VAULT/pr-reviews/" \
