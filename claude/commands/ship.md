...
5. **Garantir PR em DRAFT e persistir resumo executivo no vault** (main thread + context-keeper):
   ...
   - Identificar o repo (`basename $(git rev-parse --show-toplevel)`) e mapear pro contexto do vault:
     - Repos sob `~/www/isaac/*` → `~/.notes/pr-reviews/` (contexto `arco` vai no frontmatter; repo já embutido no nome do arquivo)
     - Outros → perguntar ao usuário o contexto, ou usar `~/.notes/0-inbox/`
   - Spawn `context-keeper` com este prompt:
     ```
     Crie nota em ~/.notes/pr-reviews/YYYY-MM-DD-<repo>-PR<N>.md (timestamp = hoje, PR<N> = número da PR).
     Frontmatter padrão do vault: date, repo, context (= "arco" para repos OlaIsaac/isaac), pr, title, url, status (draft/open), author, tags.
     Seções obrigatórias:
       ...
       ## Como testar
       ## Links
     PT-BR com acentuação correta. Wikilinks pra notas relacionadas em plans/ ou decisions/ (eixo por tipo na raiz) se aplicável.
